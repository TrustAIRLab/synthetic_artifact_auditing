...
def infer_chatglm(args, filename, res_df, instruct_list):
    ...
    for idx, (article, ref_highlights, instruct) in tqdm(enumerate(instruct_list)):
        ...
        outputs = model.generate(**inputs, do_sample=True, temperature=args.temperature, num_return_sequences=1, max_new_tokens=500)
...
def infer_mistral(args, filename, res_df, instruct_list):
    ...
    for idx, (article, ref_highlights, instruct) in tqdm(enumerate(instruct_list)):
        ...
        generated_ids = model.generate(model_inputs, max_new_tokens=500, do_sample=True, temperature=args.temperature, num_return_sequences=1)
...
def generate(args):
    ...
    dir_path = f'results/generated_data/cnn_dailymail/{args.type}'
    
    if args.type == 'shadow':
        instruct_list = generate_instruct_ref.generate_instruct_dailymail(args.type, args.num_samples)
    else:
        instruct_list = generate_instruct_target.generate_instruct_dailymail(args.type, args.num_samples)
