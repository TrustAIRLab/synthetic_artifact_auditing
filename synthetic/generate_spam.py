...
class_name = ['ham', 'spam']
class_dict = dict(zip([0, 1], class_name))
API_KEY = 'xx'
...
def parse_args():
   ...
   parser.add_argument("--label", type=int, default=0, help="spam/ham label",)
...
def generate(args):
    ...
    dir_path = f'results/generated_data/enron_spam/{args.type}'
    
    if args.type == 'shadow':
        instruct_list = generate_instruct_ref.generate_instruct_spam(args.type, args.num_samples)
    elif args.type == 'target':
        instruct_list = generate_instruct_target.generate_instruct_spam(args.type, args.num_samples)
    
    filename = os.path.join(HOME_DIR, dir_path, f'{args.llm}_{args.label}_s0_t{args.temperature}_n{args.num_samples}.csv')
    print(filename)
    
    if os.path.exists(filename):
        res_df = pd.read_csv(filename, names=['id', 'label', 'reference', 'instruct', 'text'], header=0)
        print(res_df.head())
    else:
