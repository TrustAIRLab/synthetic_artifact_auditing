...
def parse_args():
   ...
   parser.add_argument("--dataset", type=str, default='enron_spam', help="dataset name",)
...
def main(args):
    ...
    root = 'results/models/spam_detection'
        ...
        if len(args.llm_list) > 1:
            ...
            for llm, prop in zip(args.llm_list, llm_prop_distribution):
                ...
                for idx in range(num_classes):
                    ...
                    ds = ds.shuffle(args.seed).select(range(int(num_syn_samples_per_llm/num_classes)))
            ...
            for idx in range(num_classes):
                ...
                ds = ds.shuffle(args.seed).select(range(int(num_syn_samples/num_classes)))
                syn_dataset.append(ds)
    ...
    test_dataset = load_dataset("SetFit/enron_spam", split='test').shuffle(0).select(range(1000))  
