...
def main(args):
    ...
    root = 'results/models/text_summarization'
    ...
    if args.syn_prop != 0:
        ...
        if len(args.llm_list) > 1:
            ...
            for llm, prop in zip(args.llm_list, llm_prop_distribution):
                ...
                if args.dataset == 'cnn_dailymail':
                    ...
                    ds = load_dataset('csv', skiprows=1, data_files=syn_filename, 
                                    column_names=['id', 'article', 'ref_highlights', 'instruct', 'syn_highlights', 'highlights']
                                   ).remove_columns(['id', 'instruct', 'ref_highlights', 'syn_highlights'])['train']
                    ds = ds.shuffle(args.seed).select(range(int(num_syn_samples_per_llm)))
            ...
            if args.dataset == 'cnn_dailymail':
                ...
                train_syn_dataset = train_syn_dataset.shuffle(args.seed).select(range(int(num_syn_samples)))
    
    if args.dataset == 'cnn_dailymail':
        real_dataset = load_dataset('parquet', data_files={'train': os.path.join(HOME_DIR, f'data/cnn_dailymail/real_{args.type}_d_10000.parquet')})['train'] 
        test_dataset = load_dataset("cnn_dailymail", '3.0.0', split='test').shuffle(0).select([i for i in list(range(1000))]) 
    
    num_real_samples = args.num_samples - num_syn_samples
    train_real_dataset = real_dataset.shuffle(args.seed).select([i for i in list(range(num_real_samples))])  
